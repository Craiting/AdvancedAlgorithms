...
substitute = {
    ...
    ('t','a'):-3, ('c','g'):-3, ('g','c'):-3, ('g','c'):-3, ('c','a'):-1,
    ('a','c'):-1, ('a','g'):-3, ('a','a'): 5, ('c','c'): 5, ('g','g'): 5,
    ('t','t'): 5, ('g','a'):-3
}
insert_delete = { 'a':-3, 'c':-4, 'g':-2, 't':-1 }

def match_score(alpha, beta):
    if alpha == beta:
        return 5
    elif alpha == '-':
        return insert_delete[beta.lower()]
    elif beta == '-':
        return insert_delete[alpha.lower()]
    else:
        return substitute[(alpha.lower(),beta.lower())]
...
def needle(seq1, seq2):
    ...
    score = [[0 for x in range(n+1)] for x in range(m+1)] # initialize dp matrix
    ...
    while i > 0 and j > 0: # end toching the top or the left edge
        ...
        if score_current == score_diagonal + substitute[(seq1[i-1].lower(), seq2[j-1].lower())]:
            align1 += seq1[i-1]
            align2 += seq2[j-1]
            i -= 1
            j -= 1
